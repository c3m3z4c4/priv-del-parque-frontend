...
const DEFAULT_LAYOUT = [
  'weather', 'stats', 'dues', 'projects',
  'activities', 'houses', 'upcoming', 'areas',
];
...
export default function AdminDashboard() {
  ...
  const { layout, saveLayout, loaded } = useWidgetLayout('adminDashboard', DEFAULT_LAYOUT);
  ...
  const admins = users.filter(u => ['ADMIN', 'SUPER_ADMIN', 'PRESIDENTE', 'SECRETARIO', 'TESORERO'].includes(u.role));
  ...
  const widgets: WidgetDef[] = [
            ...
            {duesSummary ? (
              <div className="space-y-3">
                ...
                <div className="w-full bg-muted rounded-full h-2">
                  <div className="bg-primary h-2 rounded-full transition-all" style={{ width: `${collectedPct}%` }} />
                </div>
              </div>
            ) : (
    ...
    {
      id: 'upcoming',
      label: 'Próximos eventos y reuniones',
      render: () => (
        <Card className="shadow-card">
          <CardHeader>
            <CardTitle className="font-serif text-lg flex items-center gap-2">
